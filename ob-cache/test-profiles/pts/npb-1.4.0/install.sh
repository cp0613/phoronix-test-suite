...
C_LIB  = -lm
WTIME  = wtime.c
" > NPB3.4/NPB3.4-MPI/config/make.def

# Copy over OpenMP make for when using that...
cp NPB3.4/NPB3.4-MPI/config/make.def NPB3.4/NPB3.4-OMP/config/make.def
...
mpiexec -np \$NUM_THREADS \$HOSTFILE ./bin/\$@.x > \$LOG_FILE 2>&1
