...
C_LIB  = -lm
WTIME  = wtime.c
" > NPB3.3.1/NPB3.3-MPI/config/make.def

# Copy over OpenMP make for when using that...
cp NPB3.3.1/NPB3.3-MPI/config/make.def NPB3.3.1/NPB3.3-OMP/config/make.def
...
mpiexec -np \$NUM_THREADS \$HOSTFILE ./bin/\$@.1 > \$LOG_FILE 2>&1
